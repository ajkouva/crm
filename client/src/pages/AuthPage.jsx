...
export default function AuthPage() {
  ...
  const [form, setForm] = useState({ name:'', email:'', password:'', phone:'', role:'citizen', departmentId:'' });
  ...
  return (
    <div className="auth-page fade-up">
      ...
      <div className="auth-right">
        <div className="card" style={{ padding: '40px', width: '100%', maxWidth: '440px' }}>
          ...
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {mode === 'register' && (
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)' }}>Full Name</label>
                  <input className="input" placeholder="Your full name" value={form.name} onChange={set('name')} required />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)' }}>Role</label>
                  <CustomSelect 
                    className="input" 
                    value={form.role} 
                    onChange={set('role')}
                ...
                {needsDept && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)' }}>Department</label>
                    <CustomSelect 
                      className="input" 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)' }}>Email Address</label>
              <input className="input" type="email" placeholder="you@example.com" value={form.email} onChange={set('email')} required />
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)' }}>Password</label>
                {mode === 'login' && (
                  <button
                    type="button"
                    onClick={() => { setFpMode(true); setFpStep(1); setFpError(''); setFpDevCode(''); setFpSuccess(''); }}
                    style={{ fontSize: '0.75rem', color: 'var(--saffron)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                  >
                    Forgot Password?
                  </button>
                )}
            ...
            <button className="btn btn-primary" type="submit" disabled={loading} style={{ width: '100%', height: '44px', justifyContent: 'center', marginTop: '8px', background: 'var(--saffron)', color: 'var(--navy)', fontWeight: 700 }}>
          ...
          {fpMode && (
            <div style={{
              ...
              <div style={{
                ...
                width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)',
                ...
                {fpStep === 1 ? (
                  <form onSubmit={sendResetCode} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                      <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)', display: 'block', marginBottom: '6px' }}>Registered Email</label>
                      <input
                        className="input"
                        type="email"
                    ...
                    <button className="btn btn-primary" type="submit" disabled={fpLoading} style={{ width: '100%', height: '44px', justifyContent: 'center', background: 'var(--saffron)', color: 'var(--navy)', fontWeight: 700 }}>
                  ...
                  <form onSubmit={submitReset} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                      <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)', display: 'block', marginBottom: '6px' }}>6-Digit Reset Code</label>
                      <input
                        className="input"
                        type="text"
                        ...
                        required
                        maxLength={6}
                        style={{ letterSpacing: '8px', fontSize: '1.2rem', textAlign: 'center', fontFamily: 'monospace' }}
                        autoFocus
                      />
                    </div>
                    <div>
                      <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)', display: 'block', marginBottom: '6px' }}>New Password</label>
                      <input
                        className="input"
                        type="password"
                    ...
                    <div>
                      <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)', display: 'block', marginBottom: '6px' }}>Confirm New Password</label>
                      <input
                        className="input"
                        type="password"
                    ...
                    <button type="button" onClick={() => { setFpStep(1); setFpError(''); }} style={{ background: 'none', border: 'none', color: 'var(--text-mist)', cursor: 'pointer', fontSize: '0.8rem' }}>
                      ← Back
                    </button>
                  </form>
                )}
          ...
          {verifyMode && (
            <div style={{
              ...
              <div className="card" style={{
                background: 'var(--bg-card)', borderRadius: 'var(--radius)', padding: '40px',
                width: '100%', maxWidth: '440px', boxShadow: '0 20px 80px rgba(0,0,0,0.7)',
                border: '1px solid var(--border-color)'
              }}>
                <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                  <div style={{ 
                ...
                <form onSubmit={submitVerify} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-dim)', textAlign: 'center' }}>Enter 6-Digit Code</label>
                    <input
                      className="input"
                      type="text"
                      placeholder="000000"
                      ...
                      maxLength={6}
                      style={{ 
                        letterSpacing: '12px', fontSize: '1.8rem', textAlign: 'center', 
                        fontFamily: 'monospace', height: '60px', background: 'var(--bg-body)'
                      }}
                      autoFocus
                    />
                  </div>
                  <button className="btn btn-primary" type="submit" disabled={verifyLoading} style={{ width: '100%', height: '50px', justifyContent: 'center', background: 'var(--saffron)', color: 'var(--navy)', fontWeight: 700, fontSize: '1rem' }}>
                    {verifyLoading ? 'Verifying...' : 'Verify & Activate Account'}
                  </button>
                  <button type="button" onClick={() => setVerifyMode(false)} style={{ background: 'none', border: 'none', color: 'var(--mist)', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 500 }}>
                    Cancel
                  </button>
                </form>
              </div>
          ...
          {mode === 'login' && (
            <div style={{ marginTop: '24px', padding: '16px', background: 'var(--bg-body)', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)' }}>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-mist)', fontWeight: 600, textTransform: 'uppercase', marginBottom: '8px' }}>Demo accounts:</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <code style={{ fontSize: '0.7rem', color: 'var(--text-dim)', display: 'block' }}>citizen@demo.com / password123</code>
                <code style={{ fontSize: '0.7rem', color: 'var(--text-dim)', display: 'block' }}>officer@demo.com / password123</code>
                <code style={{ fontSize: '0.7rem', color: 'var(--text-dim)', display: 'block' }}>admin@demo.com / password123</code>
              </div>
            </div>
          )}
        </div>
